...
export default function Connections({ paddingLeft, connectionsMinimized, connectionsMaximized }: ConnectionsProps) {
  const container = useRef<HTMLDivElement>(null)
  const connectionComponentRefs = useRef<HTMLDivElement[]>([])
  const [recentlyMaximizedConnections, setRecentlyMaximizedConnections] = useState<ConnectionModel[]>([])

  useEffect(() => {
    connectionComponentRefs.current = connectionComponentRefs.current.slice(0, connectionsMaximized.length)
  }, [connectionsMaximized])

  useArrayInsertCallback<ConnectionModel>(connectionsMaximized, 'id', (connections) => {
    const connectionComponent = connectionComponentRefs.current[connectionsMaximized.indexOf(connections[0])]
    ...
    if (connectionComponent && container.current) {
      ...
      setRecentlyMaximizedConnections([...recentlyMaximizedConnections, ...connections])

      setTimeout(() => setRecentlyMaximizedConnections([]), 1000)
    }
  })
  ...
  if (!connectionsMaximized.length) {
    return (
      <EmptyMessage heading="No Connections">
        Select <strong>+</strong> at the top to create a new one.
      </EmptyMessage>
    )
  }
  ...
  return (
    <div ref={container} tw="w-full flex flex-row overflow-x-auto" css={[paddingLeft && tw`pl-2`]}>
      {connectionsMaximized.map((connection, index) => (
        ...
        >
          <Connection connection={connection} highlighted={recentlyMaximizedConnections.includes(connection)} />
        </ConnectionWrapperComponent>
      ))}
    </div>
  )
